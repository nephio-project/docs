...
You **must** do this step before connecting with VS Code because VS Code caches the environment on the server. If you want to change the values of these variables subsequently, you must restart the VM server.
...
4. We now need to install support for `go` debugging in VS Code. Trigger this by launching a debug configuration in VS Code.
