...
If not, logout and login to the VM or run `newgrp docker` to ensure the `docker` supplementary group is loaded.
...
The package is now in the `regional` directory. So you can run `set-labels` function against the package imperatively,
using `kpt fn eval`:
...
manually edit the file. Apply the kpt functions to the contents of the kpt package with a new value for the throughput
